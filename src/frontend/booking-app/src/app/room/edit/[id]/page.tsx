...
export default function EditPostPage() {
    ...
    const fetchRoomData = async () => {
        try {
            setIsLoading(true);
            const res = await fetch(`http://localhost:3000/rooms/my-rooms/${roomId}`, {
                credentials: 'include'
            });
            ...
            const cityData = locationData.cities.find(c => c.city_name === room.city);
            const cityCode = cityData?.city_code || '';
            
            const districtData = cityData?.districts.find((d: any) => d.district_name === room.district);
            const districtCode = districtData?.district_code || '';
            
            const wardData = districtData?.wards.find((w: any) => w.ward_name === room.ward);
            ...
            setFormData({
                ...
                amenities: room.roomAmenities?.map((ra: any) => ra.amenity.id.toString()) || [],
    ...
    const handleInputChange = (field: keyof FormData, value: any) => {
        ...
        if (field === 'province') {
            ...
            const district = availableDistricts.find(d => d.district_code === value);
    ...
    const buildUpdatePayload = (isDraft: boolean) => {
        ...
        const cityData = locationData.cities.find(c => c.city_code === formData.province);
        const cityName = cityData?.city_name || '';
        
        const districtData = cityData?.districts.find((d: any) => d.district_code === formData.district);
        const districtName = districtData?.district_name || '';
        
        const wardData = districtData?.wards.find((w: any) => w.ward_code === formData.ward);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : 'Không thể cập nhật tin đăng');
        } finally {
            ...
            <div className="pt-24 pb-32 px-4">
                <div className="max-w-5xl mx-auto">
                    <div className="mb-6 px-6">
                        <h2 className="text-sm font-medium text-slate-500 mb-1">Chỉnh sửa tin đăng</h2>
                        <h1 className="text-2xl font-bold text-slate-900">{STEPS[currentStep - 1]?.title}</h1>
                        <p className="text-slate-600 text-sm mt-1">{STEPS[currentStep - 1]?.desc}</p>
                    </div>
