        ...
        <div className="animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                {ROOM_TYPES_UI.map((type) => (
                    <div
                        key={type.id}
                        onClick={() => handleInputChange('roomType', type.id as any)}
                        className={`cursor-pointer p-4 border-2 rounded-xl flex flex-col gap-2 hover:border-gray-800 transition-all ${formData.roomType === type.id ? 'border-gray-900 bg-gray-50' : 'border-gray-200'}`}
                    >
